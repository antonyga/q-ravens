...
2. Based on analysis, determine what tests to run
3. Delegate to Executor to run the tests
...
When you need to delegate, clearly state which agent and what task."""
    ...
    async def process(self, state: QRavensState) -> dict[str, Any]:
        ...
        # After analysis is complete - request approval before execution
        if phase == WorkflowPhase.ANALYZING.value and analysis:
            ...
            if require_approval and not approval_response:
                ...
                approval_message = f"""Analysis Complete - Approval Required

Target: {target_url}
Pages discovered: {pages_count}
Technology stack: {tech_stack}
Has authentication: {analysis.has_authentication}

Planned tests:
  - Page Load Test (verify page loads successfully)
  - JavaScript Error Check (verify no JS errors)
  - Navigation Links Test (verify links are valid)
...
Do you approve executing these tests? (yes/no)"""
                ...
                return {
                    ...
                    "iteration_count": iteration,
                    "current_agent": self.name,
                    "messages": [AIMessage(content="Requesting approval to execute tests...")],
                }
            ...
            if approval_response:
                if not approval_response.get("approved", False):
                    ...
                    message = AIMessage(
                        ...
                        f"Analysis results are available for review."
                    )
                    return {
